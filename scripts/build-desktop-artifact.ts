...
import {
  ...
  DESKTOP_BUNDLE_ID,
  STAGED_DESKTOP_PACKAGE_NAME,
} from "@ocicode/shared/branding";
...
interface StagePackageJson {
  ...
  readonly ocicodeCommitHash: string;
...
const BuildEnvConfig = Config.all({
  platform: Config.schema(BuildPlatform, "OCICODE_DESKTOP_PLATFORM").pipe(Config.option),
  target: Config.string("OCICODE_DESKTOP_TARGET").pipe(Config.option),
  arch: Config.schema(BuildArch, "OCICODE_DESKTOP_ARCH").pipe(Config.option),
  version: Config.string("OCICODE_DESKTOP_VERSION").pipe(Config.option),
  outputDir: Config.string("OCICODE_DESKTOP_OUTPUT_DIR").pipe(Config.option),
  skipBuild: Config.boolean("OCICODE_DESKTOP_SKIP_BUILD").pipe(Config.withDefault(false)),
  keepStage: Config.boolean("OCICODE_DESKTOP_KEEP_STAGE").pipe(Config.withDefault(false)),
  signed: Config.boolean("OCICODE_DESKTOP_SIGNED").pipe(Config.withDefault(false)),
  verbose: Config.boolean("OCICODE_DESKTOP_VERBOSE").pipe(Config.withDefault(false)),
});
...
function resolveGitHubPublishConfig():
  ...
  | undefined {
  const rawRepo =
    process.env.OCICODE_DESKTOP_UPDATE_REPOSITORY?.trim() ||
...
const buildDesktopArtifact = Effect.fn("buildDesktopArtifact")(function* (
  ...
  const stageRoot = yield* mkdir({
    prefix: `ocicode-desktop-${options.platform}-stage-`,
  });
  ...
  const stagePackageJson: StagePackageJson = {
    name: STAGED_DESKTOP_PACKAGE_NAME,
    version: appVersion,
    buildVersion: appVersion,
    ocicodeCommitHash: commitHash,
    private: true,
    description: `${APP_BASE_NAME} desktop build`,
    author: APP_OWNER_NAME,
...
const buildDesktopArtifactCli = Command.make("build-desktop-artifact", {
  platform: Flag.choice("platform", BuildPlatform.literals).pipe(
    Flag.withDescription("Build platform (env: OCICODE_DESKTOP_PLATFORM)."),
    Flag.optional,
  ),
  target: Flag.string("target").pipe(
    Flag.withDescription(
      "Artifact target, for example dmg/AppImage/nsis (env: OCICODE_DESKTOP_TARGET).",
    ),
    Flag.optional,
  ),
  ...
  buildVersion: Flag.string("build-version").pipe(
    Flag.withDescription("Artifact version metadata (env: OCICODE_DESKTOP_VERSION)."),
    Flag.optional,
  ),
  outputDir: Flag.string("output-dir").pipe(
    Flag.withDescription("Output directory for artifacts (env: OCICODE_DESKTOP_OUTPUT_DIR)."),
    Flag.optional,
  ),
  skipBuild: Flag.boolean("skip-build").pipe(
    Flag.withDescription(
      "Skip `bun run build:desktop` and use existing dist artifacts (env: OCICODE_DESKTOP_SKIP_BUILD).",
    ),
    Flag.optional,
  ),
  keepStage: Flag.boolean("keep-stage").pipe(
    Flag.withDescription("Keep temporary staging files (env: OCICODE_DESKTOP_KEEP_STAGE)."),
    Flag.optional,
  ),
  signed: Flag.boolean("signed").pipe(
    Flag.withDescription(
      "Enable signing/notarization discovery; Windows uses Azure Trusted Signing (env: OCICODE_DESKTOP_SIGNED).",
    ),
    Flag.optional,
  ),
  verbose: Flag.boolean("verbose").pipe(
    Flag.withDescription("Stream subprocess stdout (env: OCICODE_DESKTOP_VERBOSE)."),
    Flag.optional,
  ),
}).pipe(
  Command.withDescription("Build a desktop artifact for OCI Code."),
