  ...
  for (const segment of segments) {
    ...
    if (segment.type === "terminal-context") {
      if (segment.context) {
        ...
        paragraph.append($createTextNode(INLINE_TERMINAL_CONTEXT_PLACEHOLDER));
      }
      continue;
    }
...
function ComposerPromptEditorInner({
  ...
  useLayoutEffect(() => {
    ...
    const nextTerminalContextIds = terminalContexts.map((context) => context.id);
    ...
    snapshotRef.current = {
      ...
      terminalContextIds: nextTerminalContextIds,
    };
  ...
  const handleEditorChange = useCallback((editorState: EditorState) => {
    editorState.read(() => {
      ...
      if (
        ...
        previousSnapshot.expandedCursor === expandedCursor &&
