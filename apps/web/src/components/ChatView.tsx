...
import { ChatHeader as ChatHeaderView } from "./chat/ChatHeader";
import { MessagesTimeline as MessagesTimelineView } from "./chat/MessagesTimeline";
import { ThreadErrorBanner as ThreadErrorBannerView } from "./chat/ThreadErrorBanner";
import { ProviderStatusBanner } from "./chat/ProviderStatusBanner";
import { ComposerPendingApprovalPanel as ComposerPendingApprovalPanelView } from "./chat/ComposerPendingApprovalPanel";
import { ComposerPendingApprovalActions as ComposerPendingApprovalActionsView } from "./chat/ComposerPendingApprovalActions";
import { ComposerPendingUserInputPanel as ComposerPendingUserInputPanelView } from "./chat/ComposerPendingUserInputPanel";
import { ComposerPlanFollowUpBanner as ComposerPlanFollowUpBannerView } from "./chat/ComposerPlanFollowUpBanner";
...
export default function ChatView({ threadId }: ChatViewProps) {
  ...
  const composerTerminalContextsRef = useRef(composerTerminalContexts);
  const [isDragOverComposer, setIsDragOverComposer] = useState(false);
  const [expandedImage, setExpandedImage] = useState<ExpandedImagePreviewState | null>(null);
  ...
  const setComposerTerminalContexts = useCallback(
    (nextContexts: typeof composerTerminalContexts) => {
      setComposerDraftTerminalContexts(threadId, nextContexts);
    },
    [setComposerDraftTerminalContexts, threadId],
  );
  ...
  const removeComposerTerminalContextFromDraft = useCallback(
    ...
    [removeComposerDraftTerminalContext, threadId],
  );
  ...
  const envMode: DraftThreadEnvMode = activeWorktreePath
    ? "worktree"
    : isLocalDraftThread
      ? (draftThread?.envMode ?? settings.defaultThreadEnvMode)
    ...
    await (async () => {
      ...
      if (!titleSeed) {
        if (firstComposerImageName) {
          ...
          titleSeed =
            buildTerminalContextPreviewTitle(composerTerminalContextsSnapshot)?.split("\n")[0] ??
            "New thread";
        }
      }
      ...
      if (
        ...
        setComposerTerminalContexts(composerTerminalContextsSnapshot);
  ...
  const onExpandTimelineImage = useCallback((preview: ExpandedImagePreviewState) => {
      ...
      <div className="chat-shell-surface flex min-h-0 min-w-0 flex-1">
        ...
        >
          <div className="relative flex min-h-0 flex-1 flex-col">
            ...
            {showScrollToBottom && (
              <div className="pointer-events-none absolute bottom-1 left-1/2 z-20 flex -translate-x-1/2 justify-center py-1.5">
                <button
                  type="button"
                  onClick={() => scrollMessagesToBottom("smooth")}
                  className="pointer-events-auto flex items-center gap-1.5 rounded-full border border-border/60 bg-card px-3 py-1 text-xs text-muted-foreground shadow-sm transition-colors hover:bg-accent hover:text-accent-foreground"
                >
              ...
              >
                {activePendingApproval ? (
                  <div className="rounded-t-[19px] border-b border-border/65 bg-muted/20">
                    <ComposerPendingApprovalPanelView
                  ...
                  <div className="rounded-t-[19px] border-b border-border/65 bg-muted/20">
                    <ComposerPendingUserInputPanelView
                  ...
                  <div className="rounded-t-[19px] border-b border-border/65 bg-muted/20">
                    <ComposerPlanFollowUpBannerView
                          ...
                          >
                            {image.previewUrl ? (
                              <button
                                ...
                                onClick={() => {
                                  const preview = buildExpandedImagePreviewForDialog(
                                    composerImages,
                                    image.id,
                                  );
                                  if (!preview) return;
                ...
                {activePendingApproval ? (
                  <div className="flex items-center justify-end gap-2 px-2.5 pb-2.5 sm:px-3 sm:pb-3">
                    <ComposerPendingApprovalActionsView
                  ...
                  >
                    ...
                    >
                      ...
                      {isComposerFooterCompact ? (
                        <CompactComposerControlsMenuView
                    ...
                    >
                      ...
                      {activePendingProgress ? (
                        ...
                        showPlanFollowUpPrompt ? (
                          ...
                          <button
                            type="submit"
                            className="flex h-9 w-9 animate-fade-in items-center justify-center rounded-full bg-primary/90 text-primary-foreground transition-all duration-150 enabled:cursor-pointer hover:bg-primary hover:scale-105 disabled:pointer-events-none disabled:opacity-30 disabled:hover:scale-100 sm:h-8 sm:w-8"
                            disabled={
