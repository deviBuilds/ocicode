...
function createBaseServerConfig(): ServerConfig {
  return {
    cwd: "/repo/project",
    keybindingsConfigPath: "/repo/project/.ocicode-keybindings.json",
    keybindings: [],
    issues: [],
    providers: [
      {
...
async function waitForMainSidebarElements(): Promise<{
  rail: HTMLButtonElement;
  sidebarContainer: HTMLElement;
}> {
  const rail = await waitForElement(
    () => document.querySelector<HTMLButtonElement>('button[aria-label="Resize Sidebar"]'),
    "Unable to find main sidebar resize rail.",
  );
  const sidebarContainer = await waitForElement(
    ...
    "Unable to find main sidebar container.",
  );
  return { rail, sidebarContainer };
}
...
describe("ChatView timeline estimator parity (full app)", () => {
  ...
  it("resizes the main sidebar when dragging the rail", async () => {
    const mounted = await mountChatView({
      viewport: { ...DEFAULT_VIEWPORT, width: 1_200 },
      snapshot: createSnapshotForTargetUser({
        targetMessageId: "msg-user-sidebar-resize-target" as MessageId,
        targetText: "sidebar resize target",
      }),
    });

    try {
      const { rail, sidebarContainer } = await waitForMainSidebarElements();
      const initialWidth = sidebarContainer.getBoundingClientRect().width;
      const railRect = rail.getBoundingClientRect();
      const startX = railRect.left + railRect.width / 2;
      const nextX = startX + 72;
      ...
      await vi.waitFor(
        () => {
          expect(sidebarContainer.getBoundingClientRect().width).toBeGreaterThan(initialWidth + 40);
        },
        { timeout: 8_000, interval: 16 },
      );
