import { type MessageId, type TurnId } from "@ocicode/contracts";
import {
  memo,
  useCallback,
  useEffect,
...
import { type TimestampFormat, formatTimestamp } from "~/timestampFormat";
import {
    ...
    >
      {row.kind === "work" &&
        (() => {
          ...
          return (
            <div className="rounded-[22px] border border-border/70 bg-card/42 px-3 py-2 shadow-xs/5">
              {showHeader && (
                <div className="mb-1.5 flex items-center justify-between gap-2 px-0.5">
                  ...
                  {hasOverflow && (
                    <button
                      type="button"
                      className="text-[10px] font-medium tracking-[0.04em] text-muted-foreground/62 transition-colors duration-150 hover:text-foreground/75"
        ...
        (() => {
          ...
          return (
            <div className="flex justify-end">
              <div className="group relative max-w-[80%] rounded-2xl rounded-br-sm border border-border bg-secondary px-4 py-3 shadow-[0_10px_24px_rgba(0,0,0,0.08)]">
                {userImages.length > 0 && (
        ...
        <div className="py-0.5 pl-1.5">
          <div className="flex items-center gap-2 pt-1 text-[11px] text-muted-foreground/70">
            <span className="inline-flex items-center gap-[3px]">
              <span className="h-1 w-1 animate-pulse-soft rounded-full bg-muted-foreground/50" />
              <span className="h-1 w-1 animate-pulse-soft rounded-full bg-muted-foreground/50 [animation-delay:200ms]" />
              <span className="h-1 w-1 animate-pulse-soft rounded-full bg-muted-foreground/50 [animation-delay:400ms]" />
            </span>
            <span>
              {row.createdAt
      ...
      <div className="flex h-full flex-col items-center justify-center gap-5 px-4">
        <div className="rounded-[28px] border border-border/70 bg-card/64 px-8 py-8 text-center shadow-[0_20px_44px_rgba(0,0,0,0.1)] supports-[backdrop-filter]:bg-card/54 supports-[backdrop-filter]:backdrop-blur-xl">
          ...
          <div className="mt-5 flex flex-wrap justify-center gap-2">
            {["Fix a bug", "Add a feature", "Refactor code", "Write tests"].map((hint) => (
              <span
                key={hint}
...
const UserMessageBody = memo(function UserMessageBody(props: {
  ...
  return (
    <pre className="whitespace-pre-wrap wrap-break-word bg-transparent text-left font-sans text-[14px] leading-6 text-foreground">
      {props.text}
    </pre>
  );
});
