...
import type { DesktopUpdateActionResult, DesktopUpdateState } from "@ocicode/contracts";
import {
  APP_BASE_NAME,
  DESKTOP_BUNDLE_ID,
...
import type { ContextMenuItem } from "@ocicode/contracts";
...
const DESKTOP_SCHEME = DESKTOP_PROTOCOL_SCHEME;
const ROOT_DIR = Path.resolve(__dirname, "../../..");
const isDevelopment = Boolean(process.env.VITE_DEV_SERVER_URL);
const APP_DISPLAY_NAME = isDevelopment ? `${APP_BASE_NAME} (Dev)` : `${APP_BASE_NAME} (Alpha)`;
const APP_USER_MODEL_ID = DESKTOP_BUNDLE_ID;
const USER_DATA_DIR_NAME = isDevelopment ? "ocicode-dev" : "ocicode";
...
function resolveEmbeddedCommitHash(): string | null {
  ...
  try {
    ...
    const parsed = JSON.parse(raw) as { ocicodeCommitHash?: unknown };
    return normalizeCommitHash(parsed.ocicodeCommitHash);
  } catch {
    return null;
  }
}
...
function resolveAboutCommitHash(): string | null {
  ...
  const envCommitHash = normalizeCommitHash(process.env.OCICODE_COMMIT_HASH);
  if (envCommitHash) {
...
function handleFatalStartupError(stage: string, error: unknown): void {
  ...
  if (!isQuitting) {
    isQuitting = true;
    dialog.showErrorBox("OCI Code failed to start", `Stage: ${stage}\n${message}${detail}`);
  }
  stopBackend();
  restoreStdIoCapture?.();
  app.quit();
...
function handleCheckForUpdatesMenuClick(): void {
  const disabledReason = getAutoUpdateDisabledReason({
    ...
    disabledByEnv: process.env.OCICODE_DISABLE_AUTO_UPDATE === "1",
  });
  if (disabledReason) {
 ...
 * (for example `~/.config/OCI Code (Alpha)` on Linux).
 *
 * We override it to a clean lowercase name (`ocicode`) for predictable
