...
projectionLayer("OrchestrationProjectionPipeline", (it) => {
  ...
  it.effect("stores message attachment references without mutating payloads", () =>
    Effect.sync(() =>
      fs.mkdtempSync(path.join(os.tmpdir(), "ocicode-projection-attachments-")),
    ).pipe(
        ...
        Effect.gen(function* () {
          ...
          yield* appendAndProject({
            ...
            payload: {
              ...
              checkpointRef: CheckpointRef.makeUnsafe(
                "refs/ocicode/checkpoints/thread-revert-files/turn/1",
              ),
              status: "ready",
              files: [],
          ...
          yield* appendAndProject({
            ...
            payload: {
              ...
              checkpointRef: CheckpointRef.makeUnsafe(
                "refs/ocicode/checkpoints/thread-revert-files/turn/2",
              ),
              status: "ready",
              files: [],
  ...
  it.effect("ignores unsafe thread ids for attachment cleanup paths", () =>
    Effect.sync(() =>
      fs.mkdtempSync(path.join(os.tmpdir(), "ocicode-projection-attachments-unsafe-")),
    ).pipe(
      Effect.flatMap((stateDir) =>
        Effect.gen(function* () {
          const projectionPipeline = yield* OrchestrationProjectionPipeline;
      ...
      Effect.gen(function* () {
        ...
        yield* appendAndProject({
          ...
          payload: {
            ...
            checkpointTurnCount: 1,
            checkpointRef: CheckpointRef.makeUnsafe(
              "refs/ocicode/checkpoints/thread-conflict/turn/1",
            ),
            status: "ready",
            files: [],
