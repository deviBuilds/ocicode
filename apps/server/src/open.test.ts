...
describe("launchDetached", () => {
  ...
  it.effect("rejects when command does not exist", () =>
    Effect.gen(function* () {
      const result = yield* launchDetached({
        command: `ocicode-no-such-command-${Date.now()}`,
        args: [],
      }).pipe(Effect.result);
...
describe("isCommandAvailable", () => {
  function withTempDir(run: (dir: string) => void): void {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "ocicode-open-"));
    try {
      run(dir);
    } finally {
...
describe("resolveAvailableEditors", () => {
  it("returns only editors whose launch commands are available", () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "ocicode-editors-"));
    try {
      ...
      assert.deepEqual(editors, ["cursor", "vscode-insiders", "vscodium", "file-manager"]);
    } finally {
      fs.rmSync(dir, { recursive: true, force: true });
    }
  });
