...
import { CliConfig, ocicodeCli, type CliConfigShape } from "./main";
...
const testLayer = Layer.mergeAll(
  Layer.succeed(CliConfig, {
    cwd: "/tmp/ocicode-test-workspace",
    fixPath: Effect.void,
...
const runCli = (
  ...
  env: Record<string, string> = { OCICODE_NO_BROWSER: "true" },
) => {
  const uniqueStateDir = `/tmp/ocicode-cli-state-${Date.now()}-${Math.random().toString(36).slice(2, 10)}`;
  return Command.runWith(ocicodeCli, { version: "0.0.0-test" })(args).pipe(
    Effect.provide(
      ConfigProvider.layer(
        ConfigProvider.fromEnv({
          env: {
            OCICODE_STATE_DIR: uniqueStateDir,
            ...env,
          },
        }),
      ),
...
it.layer(testLayer)("server CLI command", (it) => {
  it.effect("parses all CLI flags and wires scoped start/stop", () =>
    Effect.gen(function* () {
      ...
      assert.equal(resolvedConfig?.stateDir, "/tmp/ocicode-cli-state");
  ...
  it.effect("uses env fallbacks when flags are not provided", () =>
    Effect.gen(function* () {
      yield* runCli([], {
        ...
        OCICODE_PORT: "4999",
        OCICODE_HOST: "100.88.10.4",
        OCICODE_STATE_DIR: "/tmp/ocicode-env-state",
        VITE_DEV_SERVER_URL: "http://localhost:5173",
        OCICODE_NO_BROWSER: "true",
        OCICODE_AUTH_TOKEN: "env-token",
      });
      ...
      assert.equal(resolvedConfig?.stateDir, "/tmp/ocicode-env-state");
  ...
  it.effect("prefers --mode over OCICODE_MODE", () =>
    Effect.gen(function* () {
  ...
  it.effect("prefers --no-browser over OCICODE_NO_BROWSER", () =>
    Effect.gen(function* () {
      yield* runCli(["--no-browser"], {
        OCICODE_NO_BROWSER: "false",
      });
  ...
  it.effect("supports CLI and env for bootstrap/log websocket toggles", () =>
    Effect.gen(function* () {
      yield* runCli(["--auto-bootstrap-project-from-cwd"], {
        OCICODE_MODE: "desktop",
        OCICODE_LOG_WS_EVENTS: "false",
        OCICODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "false",
        OCICODE_NO_BROWSER: "true",
      });
