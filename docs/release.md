...
- Publishes the CLI package (`apps/server`, npm package `ocicode`) with OIDC trusted publishing.
...
- Repository slug source:
  - `OCICODE_DESKTOP_UPDATE_REPOSITORY` (format `owner/repo`), if set.
  - otherwise `GITHUB_REPOSITORY` from GitHub Actions.
- Temporary private-repo auth workaround:
  - set `OCICODE_DESKTOP_UPDATE_GITHUB_TOKEN` (or `GH_TOKEN`) in the desktop app runtime environment.
...
1. Confirm npm org/user owns package `ocicode` (or rename package first if needed).
